...
class Mobile_Builder_Vendor
{
    ...
    public function add_api_routes()
    {
        ...
        register_rest_route($namespace, 'directions', [
            'methods' => WP_REST_Server::READABLE,
            'callback' => [$this, 'directions'],
            'permission_callback' => '__return_true',
        ]);

        register_rest_route($namespace, 'vendor'.'/(?P<id>[\d]+)', [
            'methods' => WP_REST_Server::READABLE,
            'callback' => [$this, 'vendor'],
            'permission_callback' => '__return_true',
        ]);

        register_rest_route($namespace, 'delivery-boy-delivery-stat', [
            'methods' => WP_REST_Server::READABLE,
            'callback' => [$this, 'delivery_boy_delivery_stat'],
            'permission_callback' => '__return_true',
        ]);

        register_rest_route($namespace, 'messages-mark-read', [
            'methods' => WP_REST_Server::CREATABLE,
            'callback' => [$this, 'messages_mark_read'],
            'permission_callback' => '__return_true',
        ]);

        register_rest_route($namespace, 'messages-delete', [
            'methods' => WP_REST_Server::CREATABLE,
            'callback' => [$this, 'messages_delete'],
            'permission_callback' => '__return_true',
        ]);

        register_rest_route($namespace, 'mark-order-delivered', [
            'methods' => WP_REST_Server::CREATABLE,
    ...
    public function vendors($request)
    {
        ...
        foreach ($stores as $key => $value) {
            ...
            $vendor_stores[] = array_merge($store, [
                'id' => $key,
        ...
        foreach ($vendor_stores as $key => $value) {
            $vendor_stores[$key] = array_merge($value, [
    ...
    public function mbd_product_list_geo_location_filter_post_clauses($args, $wp_query)
    {
        ...
        if (!empty($radius_lat) && !empty($radius_lng) && !empty($radius_range)) {
            ...
            $user_args = [
                'role__in' => apply_filters('wcfmmp_allwoed_vendor_user_roles', ['wcfm_vendor']),
                'count_total' => false,
            ...
            foreach ($delivery_ids as $delivery_id) {
                ...
                if (!empty($delivery_details)) {
                    foreach ($delivery_details as $delivery_detail) {
                        ...
                        $wpdb->update("{$wpdb->prefix}wcfm_delivery_orders", [
                            'delivery_status' => 'delivered',
                            'delivery_date' => date('Y-m-d H:i:s', current_time('timestamp', 0)),
                        ], ['ID' => $delivery_id], ['%s', '%s'], ['%d']);
