...
class Test_Plugin_Base extends \WP_UnitTestCase {
	...
	public function test_relative_path() {
		$this->assertEquals( 'plugins/frego-mobile-builder', $this->plugin->relative_path( '/var/www/html/wp-content/plugins/frego-mobile-builder', 'wp-content', '/' ) );
		$this->assertEquals( 'themes/twentysixteen/plugins/frego-mobile-builder', $this->plugin->relative_path( '/var/www/html/wp-content/themes/twentysixteen/plugins/frego-mobile-builder', 'wp-content', '/' ) );
	}
	...
	public function test_trigger_warning() {
		...
		set_error_handler(
			function( $errno, $errstr ) use ( $obj ) {
				$obj->assertEquals( 'FregoMobileBuilder\Plugin: Param is 0!', $errstr );
	...
	public function test_asset_version() {
		$mock = $this->getMockBuilder( 'FregoMobileBuilder\Plugin' )
			->setMethods(
				[
					'is_debug',
					'is_script_debug',
