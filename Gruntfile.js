...
module.exports = function ( grunt ) {
	...
	grunt.initConfig( {
		...
		shell: {
			...
			create_build_zip: {
				command: 'if [ ! -e build ]; then echo "Run grunt build first."; exit 1; fi; if [ -e frego-mobile-builder.zip ]; then rm frego-mobile-builder.zip; fi; cd build; zip -r ../frego-mobile-builder.zip .; cd ..; echo; echo "ZIP of build: $(pwd)/frego-mobile-builder.zip"',
			},
		},
		...
		wp_deploy: {
			deploy: {
				options: {
					plugin_slug: 'frego-mobile-builder',
					build_dir: 'build',
